...
import { affiliateURL, baseURL, postReq } from "../../assets/auth/jwtService"
    ...
    if (checkForm) {
      ...
      if (test) {
        if (!emailCheck) {
          ...
          // fetch('https://4c16-2402-e280-3d9c-20d-93dd-c121-5a05-b109.ngrok-free.app/signupaffliate', {
          //   method: 'POST',
          //   body: formData
          // })
          // .then((res) => {
          ...
          postReq("signupaffliate", formData, affiliateURL)
            .then((res) => {
              ...
              setToken(tokenValue)
              if (res.status === 201) {
                toast.success('Sign Up Sucesful')
